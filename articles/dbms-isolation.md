---
title: "自作DBMSに必要なトランザクション知識（導入・Isolation編）"
emoji: "🐘"
type: "tech"
...
人類が生み出したソフトウェアの叡智の結晶の1つであるDBMS、その根幹に鎮座するトランザクション理論は、ユーザー視点での「あって当たり前」という感覚とは裏腹に、様々な概念や歴史的な背景が入り乱れ、全体像を把握することが大変困難です。
従って、本記事に至っても、筆者の理解不足が含まれている可能性はあり、その際はお手柔らかにご指摘ください。

# トランザクションとは

データベースにおけるトランザクションとは、一言で言うと「一連の読み書き操作をまとめたもの」です。
耳タコな例の銀行の送金処理については今更説明しませんが、一連の操作が「全て成功する」か「全て失敗する」かのどちらかであることが求められるケースは現実に多く、これがトランザクションという概念が必要になる基本的な動機です。
...
トランザクションが保証すべき性質として、ACID特性が広く知られています。
...
これは例えば、「口座残高がマイナスにならない」、「ユーザーIDは重複しない」といった、ビジネスロジックの整合性を保証するとされていますが、具体的にどういった整合性を要求するかはアプリケーション側の責務です。
DBMSの提供する型システムや制約はConsistencyを支援するかもしれませんが、汎用的なトランザクション処理からしたらその具体は知ったこっちゃないわけです。
...
これらはもちろん相互に関連しているものの、各々どういった保証でどういった理論の元で成り立っているのかを分解して考えることで、トランザクション理論の全体像を捉えやすくなると筆者は考えています。
...
Durabilityを考えるともう少しややこしくなるのですが、本記事ではこの程度に留めておきます。
...
もちろん、隕石が落ちてきたらどうしようもないので、トランザクション文脈では、変更内容が不揮発性ストレージ（ディスクやSSDなど）に記録された時点でDurabilityが保証されると考えることが多いはずです（単一DBの場合）。

最も単純な実現方法は、Commitされるたびにその内容をディスクに書き込むことに思われます。
しかし、ディスクアクセスはメモリアクセスに比べてすこぶる遅いため、現実的ではないのは見るも明らかです。
そのため、現実のDBMSでは、メモリ（バッファプール）上でデータの読み書きを行いますが、揮発性メモリを使うトレードオフとして復旧のためのアルゴリズムが要求されます。
WAL（Write-Ahead Log）やARIESという仕組み・アルゴリズムが典型的ですが、本記事では次のIsolationに焦点を当てるため、こちらもこの程度に留めておきます。

## Isolation （独立性/隔離性）

Isolationは、複数のトランザクションが同時に実行される状況において互いに影響を与えないことを保証する性質です。

DBMS側で制御し、同時に実行されているトランザクションが常に一つになるように他のトランザクションを待たせておけば（逐次実行）、少なくともIsolationの問題は発生しませんが、これまたパフォーマンス上現実的ではないことは明白です。
そこで、通常は複数のトランザクションを並行（Concurrent）に実行し、各トランザクションの個々の操作は時系列的に入り混じることになります。
しかし、デタラメでいいはずもないので、どのような順序なら許容範囲なのかということが今後の話の中心となります。
...
あるトランザクションの集合に対して、その操作群を（各トランザクション内での順序を保ったまま）並び替えた列をスケジュールと定義します。
...
Conflict Serializabilityは、「競合する操作の処理順序」に注目する考え方です。
...
ここまででConflict Serializableを判定する理屈は分かりました。
しかしながら、実際のDBMSでは、トランザクションはリアルタイムに流れ込み、Commit/Abortされるまでどのような操作が行われるかも分かりません。
したがって「あとから見れば Conflict Serializable だった」という結果を、オンラインで（実行中から）保証する仕組みが必要です。

そこで登場するのがロック機構、古典的な Two-Phase Lock (2PL)です。
またまた展開が飛躍してしまいますが、2PL に従えば得られるスケジュールは必ず Conflict Serializable になる、という定理が知られています。

## Shared(共有)/Exclusive(排他) Lock

まずロックの基本形を確認します。

|          | S ロックを保持中   | X ロックを保持中 |
| -------- | ------------------ | ---------------- |
| S を取得 | 互換あり（取得可） | 競合（待機）     |
| X を取得 | 競合（待機）       | 競合（待機）     |

- Shared (S) ロック — 読み取り時に取得。互いに共有可。
- Exclusive (X) ロック — 書き込み時に取得。他の S/X と競合。
- 競合した場合は、ロックが解放されるまで待機する。

この辺りはよく見ると思います。Conflictの定義とも似ています。

## Two Phase Lock

2PL はロック取得・解放を 二段階の順序で行われるように に制約します。
いったん縮小期に入ったら二度と成長期へ戻らないというルールです。

1. 成長期 — ロックを取得してよい（解放は禁止）
2. 縮小期 — ロックを解放してよい（取得は禁止）

![](/images/dbms-isolation/tx011.png)

### 証明

2PL に従えば得られるスケジュールは必ず Conflict Serializable になる、ということは背理法で証明できます。
...
2PL は厳格なロック制御のおかげで整合性は保てるものの、 そのぶんデッドロックが発生しやすい という弱点があります。  
デッドロック自体の説明はよく聞くと思うので割愛しますが、 潜在的に起こる 以上は 検知の仕組み と 対応方法の決め が必要です。

# Cascading Abort

さて、2PLに従っていればConflict Serializabilityを満たすことができ、Isolationを保証する実装が行えそうです。
しかし、ここまでの話では、トランザクションがAbortされることを考えていませんでした。

例として、次の逐次スケジュールと同形の並列実行を考えます。
...
先ほどの定義だと、ロックの取得と解放の話しかしていなかったため、以下のようなパターンが考えられます。

1. $T_{1}$ が $X$ を更新
2. $T_{2}$ がその新しい $X$ を読み取る
3. その後 $T_{1}$ が **Abort**

![](/images/dbms-isolation/tx012.png)

すると $T_{2}$ は “消えるはずの値” を読んでいるため、Abortしなければ整合性が保てません。
更に$T_{2}$をAbortすると、その処理に依存する更に後続のトランザクションも連鎖的(Cascading)にAbortする必要が生じます。
これをCascading Abortと言いますが、できれば回避したい事態です。

## Strict Two Phase Lock

CascadingAbortを防ぐには、Two Phase Lockにもう一段制約を加えます。

> ロックはトランザクション終了時まで解放しない。

![](/images/dbms-isolation/tx013.png)

そうすることで、解放したがその後Abortされると言うことを防げます。

![](/images/dbms-isolation/tx014.png)

実際の DBMS では「いつロックを解放するか」を汎用的に決めづらいため、結果として **Strict（あるいは Rigorous）2PL** が標準的な挙動になっていることが多いです。

また、細かい話をすると、Writeのロックに関してその制約を求めるものをStrict 2PL、全てのロックに関して求めるものをRegiou Strict 2PLと言います。
