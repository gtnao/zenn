...
- 筆者はembulk-coreにはコントリビュートしているわけではないので、間違いなどがあればお気軽にご指摘ください。
...
また、この辺りの詳しい話はコアのメンテナーをしているdmikurubeさんの記事に詳しくまとまっています。
...
![](https://storage.googleapis.com/zenn-user-upload/7365bd4184bd-20231128.png)
...
https://github.com/embulk/embulk/blob/v0.11.2/embulk-core/src/main/java/org/embulk/spi/FileInputRunner.java
...
https://github.com/embulk/embulk/blob/v0.11.2/embulk-core/src/main/java/org/embulk/cli/Main.java
...
例えばinput部分であれば、前述の`FileInputRunner`の`transaction`が呼ばれ、その中でFileInputPlugin,DecoderPlugin,ParserPluginの`transaction`が呼ばれていきます。
...
https://github.com/embulk/embulk-output-jdbc/blob/d2145e3ac87d839c382ab25a540d450257808c59/embulk-output-jdbc/src/main/java/org/embulk/output/jdbc/AbstractJdbcOutputPlugin.java#L463
...
`DirectExecutor`,`ScatterExecutor`に関しては、以下のsonotsさんの記事でも詳しく解説されています。
...
データの処理と逆の流れでリレーしていったことで、最終的にはデータの処理の順に`PageOutput`がネストされています。

![](https://storage.googleapis.com/zenn-user-upload/8e85e8339774-20231206.png)
...
各`PageOutput`は後続の`PageOutput`を内包しているため、各プラグインにおける役目を終えると、`Page`を生成して後続の`PageOutput`に`add`していきます。
最終的にはOutputPluginに到達して、その`add`メソッドはデータロードの処理を実装しています。

流れは以下のシーケンス図のようになります。

![](https://storage.googleapis.com/zenn-user-upload/fba568a658ab-20231206.png)

つまり、まずデータ元のデータを全件取得して後続に渡して処理していくような逐次的処理ではなく、データの取得中にも順次後続の処理を行っていくようなストリーミング処理となっています。
...
`buffer`は32KBのバイト列で主に以下のようにバイトが割り当てられています。
...
以下のように各プラグイン間においてのデータ受け渡しのたびに生成されます。
